...
import { Zone, ZoneStatus, CrowdSnapshot, Alert, Recommendation, CrowdZone } from '../types';
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const enrichedZones: CrowdZone[] = crowdData.map((c) => {
        ...
        const history = existing ? [...existing.history.slice(-19), c.density] : Array(20).fill(c.density);
        const waitHistory = existing ? [...existing.waitHistory.slice(-19), c.queue_time] : Array(20).fill(c.queue_time);
        const trend = history.length >= 2 
          ? history[history.length - 1] > history[history.length - 2] + 0.02 ? 'up' 
            : history[history.length - 1] < history[history.length - 2] - 0.02 ? 'down' : 'stable'
          : 'stable';
