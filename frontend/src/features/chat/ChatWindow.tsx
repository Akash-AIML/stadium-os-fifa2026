...
function WelcomeView({ onChipClick, t }: Readonly<WelcomeViewProps>) {
  return (
    <motion.div
      key="welcome"
      ...
      >
        <AnimatePresence mode="popLayout">
          ...
          {state.chat_history.map((msg) => {
            const isLast = state.chat_history.length > 0 && msg.id === state.chat_history[state.chat_history.length - 1].id;
